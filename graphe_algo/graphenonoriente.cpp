...
void GrapheNonOriente:: trier()
{
    ...
    for(int i=1; i<=d_m ; i++)
    {
        ...
        {
            if((d_aret[j+1].getValeur()<d_aret[j].getValeur())|| ((d_aret[j+1].getValeur()==d_aret[j].getValeur()) && (d_aret[j+1].getSommetDepart().getNumero()<d_aret[j].getSommetDepart().getNumero()) ) || ((d_aret[j+1].getValeur()==d_aret[j].getValeur()) && (d_aret[j+1].getSommetArrive().getNumero()<d_aret[j].getSommetArrive().getNumero())) )
            {
                x.setArc(d_aret[j]);
